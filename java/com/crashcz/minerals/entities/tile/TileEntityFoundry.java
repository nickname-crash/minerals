...
public class TileEntityFoundry extends TileEntity implements ITickable
{
	...
	{	
		...
		{
			--this.burnTime;
			Foundry.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = FoundryRecipes.getInstance().getFoundryResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = FoundryRecipes.getInstance().getFoundryResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
