...
public class TileEntityFoundry extends TileEntity implements ITickable, IInventory
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.foundry";
	}
	@Override
	...
	private boolean canSmelt()
	{
		...
		{
			ItemStack result = FoundryRecipes.getInstance().getFoundryResult((ItemStack)this.inventory.get(0), (ItemStack)this.inventory.get(1));
			if(result.isEmpty())
				return false;
			else
			{
				...
				if(output.isEmpty())
					return true;
				if(!output.isItemEqualIgnoreDurability(result))
					return false;
	...
	public static int getItemBurnTime(ItemStack fuel)
	{
		...
		else
		{
			...
			if(item instanceof ItemTool && "WOOD".equals(((ItemTool)item).getToolMaterialName()))
					return 200;
			if(item instanceof ItemSword && "WOOD".equals(((ItemTool)item).getToolMaterialName()))
					return 200;
			if(item instanceof ItemHoe && "WOOD".equals(((ItemTool)item).getToolMaterialName()))
					return 200;
			if(item == Items.STICK)
				return 100;
			if(item == Items.COAL)
